import { z as zoom, i as identity, t as transform } from "./d3-zoom.mjs";
import { s as select, p as pointer } from "./d3-selection.mjs";
import { a as drag } from "./d3-drag.mjs";
import { e as interpolateZoom, f as interpolate } from "./d3-interpolate.mjs";
const errorMessages = {
...
function XYPanZoom({ domNode, minZoom, maxZoom, translateExtent, viewport, onPanZoom, onPanZoomStart, onPanZoomEnd, onDraggingChange }) {
  ...
  async function setTransform(transform2, options) {
    if (d3Selection) {
      return new Promise((resolve) => {
        d3ZoomInstance?.interpolate(options?.interpolate === "linear" ? interpolate : interpolateZoom).transform(getD3Transition(d3Selection, options?.duration, options?.ease, () => resolve(true)), transform2);
      });
    }
    return false;
  }
  ...
  async function scaleTo(zoom2, options) {
    if (d3Selection) {
      return new Promise((resolve) => {
        d3ZoomInstance?.interpolate(options?.interpolate === "linear" ? interpolate : interpolateZoom).scaleTo(getD3Transition(d3Selection, options?.duration, options?.ease, () => resolve(true)), zoom2);
      });
    }
    return false;
  }
...
export {
  areSetsEqual as $,
  isNumeric as A,
  getStraightPath as B,
  ConnectionMode as C,
  isEdgeVisible as D,
  getConnectionStatus as E,
  mergeAriaLabelConfig as F,
  withResolvers as G,
  getNodesBounds as H,
  isRectObject as I,
  getOverlappingArea as J,
  nodeToRect as K,
  getElementsToRemove as L,
  MarkerType as M,
  getBoundsOfRects as N,
  getInternalNodesBounds as O,
  Position as P,
  getEventPosition as Q,
  ResizeControlVariant as R,
  SelectionMode as S,
  pointToRendererPoint as T,
  rendererPointToPoint as U,
  getBezierEdgeCenter as V,
  getViewportForBounds as W,
  XYHandle as X,
  evaluateAbsolutePosition as Y,
  getDimensions as Z,
  calcAutoPan as _,
  addEdge as a,
  isNodeBase as a0,
  ...
  getNodeDimensions as b,
  createDevWarn as c,
  createMarkerIds as d,
  errorMessages as e,
  getSmoothStepPath as f,
  getHostForElement as g,
  getBezierPath as h,
  isMouseEvent as i,
  getEdgePosition as j,
  getElevatedEdgeZIndex as k,
  getMarkerId as l,
  infiniteExtent as m,
  nodeHasDimensions as n,
  PanOnScrollMode as o,
  ConnectionLineType as p,
  isMacOs as q,
  XYMinimap as r,
  XYResizer as s,
  XYPanZoom as t,
  XYDrag as u,
  snapPosition as v,
  calculateNodePosition as w,
  getNodesInside as x,
  isInputDOMNode as y,
  elementSelectionKeys as z
};
